...
*dedup_mode* (*CCACHE_DEDUP* or *CCACHE_NODEDUP*, see <<_boolean_values,Boolean values>> above)::

    If true, the dedup mode will be used. The default is false. See
    <<_the_dedup_mode,THE DEDUP MODE>>.
...
In the dedup(lication) mode, a "binary" hash is formed of all output files.

Based on the hash, compilations leading to the exact same result will only be
stored once in the cache. This will save space, at the expense of extra time.
