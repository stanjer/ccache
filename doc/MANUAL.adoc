...
*store_command* (*CCACHE_STORECOMMAND* or *CCACHE_NOSTORECOMMAND*, see <<_boolean_values,Boolean values>> above)::

    Store the original compiler command in the cache, next to the object.
    This feature can be useful when troubleshooting cache hits and misses.

    The first line is the full command line, with the original arguments.
    Line two is the input (source) and line three is the output (object).
