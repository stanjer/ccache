...
*reflink* (*CCACHE_REFLINK* or *CCACHE_NOREFLINK*, see <<_boolean_values,Boolean values>> above)::

    If true, ccache will attempt to clone (Copy-on-Write) where available
    when creating the compiler output rather than using a file copy. Reflinks
    are never made for compressed cache files. This means that you should not
    enable compression if you want to use reflinks. The default is false.
